...
int main(void) {
    ...
    // char c;
    // int i;
    // const char const_Buffer[] ={"777 9999 0x7779898 -5.7898e+2 V 0x1a79f9"};
    // const char const_Format[] = {"%hd %u %X %f %c %i"};
    // s21_sscanf(const_Buffer, const_Format, &day, &yars, &test, &flot, &c, &i);
...
int s21_sscanf(const char const_Buffer[], const char const_Format[], ...) {
    ...
    parser_const_Format(const_Format, &format_i, i); // ищем форматы и флаги и все остальное
    parser_tip_arg_get_funkshin(const_Buffer, const_Format, &format_i); // ищем нужные фунции в зависимости от типа переменной
                                                                        // и запускаем её (возможно с учетом флагом)
    
    if (format_i.arg_int == 1 && format_i.dlina_h == 0 && format_i.dlina_l == 0){
        int d = format_i.znachenie_int;
        int *d_adress = (int *)va_arg(ap, int *);
        *d_adress = d;
    }

    if (format_i.arg_int == 1 && format_i.dlina_h == 1 && format_i.dlina_l == 0){
    ...
    if (format_i.arg_int == 1 && format_i.dlina_h == 0 && format_i.dlina_l == 1){
    ...
    if (format_i.arg_f == 1){
        ...
        *a_adress = format_i.znachenie_float;
        printf("a == %lf\n" , format_i.znachenie_float); //после запятой не точно, доделать преобразование в флот
        printf("a == %lf\n" , *a_adress);
    }
