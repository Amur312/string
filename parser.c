...
void parser_const_Format(const char *const_Format, struct pars_format *format_i, int pointer){
    ...
    for (int i = pointer; i < n; i ++){ // parsim Format
        if(const_Format[i] != '%'){
            continue;
        }
        int b = i + 1;
...
void parser_tip_arg_get_funkshin(const char *const_Buffer, const char *const_Format, struct pars_format *format_i) {
    ...
    if (format_i->arg_int == 1 && format_i->dlina_h == 0 && format_i->dlina_l == 0){ //int
        funkshin_for_int(const_Buffer, const_Format, format_i);
    }
    if (format_i->arg_int == 1 && format_i->dlina_h == 1 && format_i->dlina_l == 0){ //short int 
        funkshin_for_int_h(const_Buffer, const_Format, format_i);
    }
    if (format_i->arg_int == 1 && format_i->dlina_h == 0 && format_i->dlina_l == 1){ // long int 
        funkshin_for_int_l(const_Buffer, const_Format, format_i);
    }
    if (format_i->arg_c == 1 ){
        funkshin_for_c(const_Buffer, const_Format, format_i);
    }
    ...
    if (format_i->arg_e == 1){
        funkshin_for_e(const_Buffer, const_Format, format_i);
    }
    if (format_i->arg_E == 1){
        funkshin_for_E(const_Buffer, const_Format, format_i);
    }
    ...
    if (format_i->arg_g == 1){
        funkshin_for_g(const_Buffer, const_Format, format_i);
    }
    if (format_i->arg_G == 1){
        funkshin_for_G(const_Buffer, const_Format, format_i);
    }
    if (format_i->arg_o == 1){
        funkshin_for_o(const_Buffer, const_Format, format_i);
    }
    if (format_i->arg_u == 1){
        funkshin_for_u(const_Buffer, const_Format, format_i);
    }
    if (format_i->arg_s == 1){
        funkshin_for_s(const_Buffer, const_Format, format_i);
    }
    if (format_i->arg_x == 1){
        funkshin_for_x(const_Buffer, const_Format, format_i);
    }
    if (format_i->arg_X == 1){
        funkshin_for_X(const_Buffer, const_Format, format_i);
    }
